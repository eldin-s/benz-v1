import React from "react";

const Oprema = () => {
  return (
    <div className="bg-bgShade p-6 mt-8 rounded-2xl">
      <h4 className="text-xl py-4 px-2 mb-4 tracking-wider border-b border-primary ">
        Oprema
      </h4>
      <div className="flex justify-between flex-wrap gap-x-16 gap-y-6">
        <div>
          <p>Branici u boji auta</p>
          <p>Servo volan</p>
          <p>Multifuncionalni volan</p>
          <p>Daljinsko zakljucavanje</p>
          <p>Putni racunar</p>
          <p>Tonirana stakla</p>
          <p>Elektricni podizaci</p>
          <p>Elektricni retrovizori</p>
          <p>Grejaci retrovizora</p>
          <p>Sedista po visini</p>
          <p>Elektro podesiva sedista</p>
          <p>Grejanje sedista</p>
          <p>Svetla za maglu</p>
          <p>Senzori za svetla</p>
          <p>Senzori za kisu</p>
          <p>Parking senzori</p>
        </div>
        <div>
          <p>Branici u boji auta</p>
          <p>Servo volan</p>
          <p>Multifuncionalni volan</p>
          <p>Daljinsko zakljucavanje</p>
          <p>Putni racunar</p>
          <p>Tonirana stakla</p>
          <p>Elektricni podizaci</p>
          <p>Elektricni retrovizori</p>
          <p>Grejaci retrovizora</p>
          <p>Sedista po visini</p>
          <p>Elektro podesiva sedista</p>
          <p>Grejanje sedista</p>
          <p>Svetla za maglu</p>
          <p>Senzori za svetla</p>
          <p>Senzori za kisu</p>
          <p>Parking senzori</p>
        </div>
        <div>
          <p>Branici u boji auta</p>
          <p>Servo volan</p>
          <p>Multifuncionalni volan</p>
          <p>Daljinsko zakljucavanje</p>
          <p>Putni racunar</p>
          <p>Tonirana stakla</p>
          <p>Elektricni podizaci</p>
          <p>Elektricni retrovizori</p>
          <p>Grejaci retrovizora</p>
          <p>Sedista po visini</p>
          <p>Elektro podesiva sedista</p>
          <p>Grejanje sedista</p>
          <p>Svetla za maglu</p>
          <p>Senzori za svetla</p>
          <p>Senzori za kisu</p>
          <p>Parking senzori</p>
        </div>
      </div>
    </div>
  );
};

export default Oprema;
